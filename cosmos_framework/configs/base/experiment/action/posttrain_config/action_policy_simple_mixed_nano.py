...
"""``action_policy_simple_mixed_nano`` — Cosmos3-Nano G1 "simple" MULTITASK (14-task) SFT recipe.

Equal-weight blend of 14 G1 "simple" whole-body tasks (each 20-ep: 18 train / 2 val),
one policy, one 36-D action head (all share ``domain_name="g1_simple"``).
...
- Equal ratio (1 each) == size-proportional here anyway, since every task is
  subsampled to 18 train eps. No task dominates.
- Each task self-normalizes (minmax on its own 20-ep stats) and is served with its own
...
on purpose); and per-stream 8 ranks x 1 worker = 8 shards <= 18 train eps (no shard hang).

Usage (1 node, 8 GPU): select experiment=action_policy_simple_mixed_nano; TOML sets max_iter.
"""
...
def _stream(task: str, split: str, shuffle: bool, cfg_drop: float, in_order: bool):
    ...
    return dict(
        ...
        dataloader=L(RankPartitionedDataLoader)(
            batch_size=4,
            in_order=in_order,
            num_workers=1,  # 8 ranks x 1 = 8 shards <= 18 train eps; 14 streams => keep procs low
            ...
            datasets={
                task: dict(
                    ...
                    dataset=L(get_action_simple_sft_dataset)(
                        root=f"${{oc.env:PSI_HOME}}/data/simple/{task}_v30_20ep/{split}",
                        fps=50.0,
                        chunk_length=32,
                        mode="policy",
                        use_state=True,
...
action_policy_simple_mixed_nano = LazyDict(
    dict(
        defaults=[
        ...
        job=dict(
            project="cosmos3",
            group="action_sft",
            name="action_policy_simple_mixed_nano",
            wandb_mode="disabled",
        ),
        ...
        # RankPartitionedDataLoader that shards its 18 train eps over all 8 ranks.
...
action_policy_simple_mixed_nano["model"]["config"]["max_action_dim"] = 36
action_policy_simple_mixed_nano["model"]["config"]["tokenizer"]["encode_exact_durations"] = [33]
action_policy_simple_mixed_nano["model"]["config"]["max_num_tokens_after_packing"] = -1
action_policy_simple_mixed_nano["model"]["config"]["rectified_flow_training_config"]["loss_scale"] = 10.0


for _item in [action_policy_simple_mixed_nano]:
