...
class DistributedCheckpointer(AbstractCheckpointer):
    ...
    def save(
        ...
        # Keep full (resumable) state only for the latest checkpoint; strip older ones to
        # model weights. Older checkpoints are only pruned (never the just-saved one), so
        # this is safe even for async saves still flushing the current checkpoint.
        if self.keep_only_model_for_older_checkpoints:
            self._prune_older_checkpoints_to_model_only(checkpoint_file)

    def _prune_older_checkpoints_to_model_only(self, current_checkpoint_file: str) -> None:
        """Trim older checkpoints: strip to model weights, cap the count, keep milestones.

        For every ``iter_*`` except the latest (``current_checkpoint_file``, kept fully
        resumable):
          * a checkpoint beyond the ``MAX_RECENT`` most recent whose step is NOT a multiple
            of ``MILESTONE_STEP`` is deleted entirely;
          * every retained older checkpoint is stripped to model weights only (delete the
            optim/scheduler/trainer/dataloader subdirs) — weights for eval/export/archival.

        Net on disk: 1 full latest + up to ``MAX_RECENT`` recent model-only checkpoints +
        every ``MILESTONE_STEP``-step model-only checkpoint. Rank-0 + local-filesystem only;
        a pure-S3 ``save_dirname`` (or missing dir) is skipped.
        """
        MAX_RECENT = 5  # keep at most this many of the latest checkpoints (incl. the full one)
        MILESTONE_STEP = 10000  # checkpoints at a multiple of this step are always retained
        if not distributed.is_rank0():
            return
        save_dir = self.save_dirname
        if not isinstance(save_dir, str) or not os.path.isdir(save_dir):
            return  # e.g. s3:// save_dirname — nothing local to prune
        # Newest-first; the MAX_RECENT most recent are retained (as model-only).
        iters = sorted(
            (n for n in os.listdir(save_dir) if re.fullmatch(r"iter_\d{9}", n)),
            reverse=True,
        )
        keep_recent = set(iters[:MAX_RECENT])
        stripped, deleted = [], []
        for name in iters:
            if name == current_checkpoint_file:
                continue  # latest stays fully resumable
            iter_dir = os.path.join(save_dir, name)
            if not os.path.isdir(iter_dir):
                continue
            step = int(name[len("iter_"):])
            # Delete entirely if it is neither among the recent kept nor a milestone.
            if name not in keep_recent and step % MILESTONE_STEP != 0:
                try:
                    shutil.rmtree(iter_dir)
                    deleted.append(name)
                except OSError as e:
                    log.warning(f"prune: could not remove {iter_dir}: {e}")
                continue
            # Otherwise keep it, stripped to model weights only.
            removed_any = False
            for entry in os.listdir(iter_dir):
                if entry == "model":
                    continue
                path = os.path.join(iter_dir, entry)
                try:
                    shutil.rmtree(path) if os.path.isdir(path) else os.remove(path)
                    removed_any = True
                except OSError as e:
                    log.warning(f"prune: could not remove {path}: {e}")
            if removed_any:
                stripped.append(name)
        if stripped or deleted:
            log.info(
                f"Checkpoint prune (kept full: {current_checkpoint_file}): stripped "
                f"{len(stripped)} to model-only, deleted {len(deleted)} "
                f"(retain {MAX_RECENT} latest + step%{MILESTONE_STEP}==0)."
            )
